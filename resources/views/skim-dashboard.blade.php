...
@section('title', 'Laporan SKIM · PDF & Word')
    ...
    <section id="section-upload">
        <div x-data="uploadZone()" class="space-y-5">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 bg-teal-50/60">
                    <h2 class="text-base font-semibold text-gov-blue flex items-center gap-2">
                        ...
                        Upload Dokumen SKIM — Surat Keterangan Keimigrasian
                    </h2>
                    <p class="text-xs text-slate-500 mt-0.5">Mendukung upload file PDF  (.docx). Maksimal 20MB/file</p>
                </div>
                <div class="p-6">
                    <form id="upload-form" action="{{ route('skim.upload') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="upload-zone border-2 border-dashed border-teal-300/60 rounded-xl p-10 text-center cursor-pointer"
                            ...
                            <div class="flex flex-col items-center gap-3">
                                <div class="w-16 h-16 rounded-full bg-teal-50 flex items-center justify-center" :class="{ 'bg-teal-100': isDragging }">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-400 text-xs mt-1">Surat Keterangan Keimigrasian (Pasal 19)</p>
                                </div>
                            </div>
                            <input type="file" name="docs[]" multiple accept=".pdf,.docx,.doc" x-ref="fileInput" class="hidden" @change="handleFileSelect($event)" id="skim-file-input">
                        </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-48 overflow-y-auto pr-1">
                                <template x-for="(file, index) in files" :key="index">
                                    <div class="file-tag flex items-center gap-2 px-3 py-2 rounded-lg">
                                        <svg class="w-4 h-4 text-teal-500 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clip-rule="evenodd"/></svg>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-xs font-medium text-teal-700 truncate" x-text="file.name"></p>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #14b8a6;">
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total SKIM</p>
                    <p class="text-2xl font-bold text-teal-600 mt-1">{{ $totalCount }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #3b82f6;">
                    ...
                    <p class="text-2xl font-bold text-blue-600 mt-1">{{ \App\Models\ParsedSkimDocument::where('jenis_kelamin','L')->count() }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #ec4899;">
                    ...
                    <p class="text-2xl font-bold text-pink-500 mt-1">{{ \App\Models\ParsedSkimDocument::where('jenis_kelamin','P')->count() }}</p>
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="px-6 py-4 border-b border-slate-100 bg-teal-50/60">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <form method="GET" action="{{ route('skim.index') }}" class="flex-1 sm:flex-none">
                            <input type="hidden" name="tab" value="data">
                            <div class="relative">
                                <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama, register, niora..."
                        ...
                        @if($totalCount > 0)
                        <a href="{{ route('skim.export') }}" class="btn-primary flex items-center gap-2 px-4 py-2 text-white text-xs font-semibold rounded-lg shadow-md whitespace-nowrap">
                ...
                <table class="w-full text-xs">
                    <thead>
                        <tr class="bg-gov-blue text-white">
                            ...
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Niora</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Status Sipil</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Kewarganegaraan</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Pekerjaan</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">No. Paspor</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Jenis Keimigrasian</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-slate-50/50' }}">
                            ...
                            <td class="px-3 py-2.5 font-mono text-slate-700 whitespace-nowrap">{{ $doc->niora ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap">{{ $doc->status_sipil ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap">{{ $doc->kewarganegaraan ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap">{{ $doc->pekerjaan ?? '-' }}</td>
                            <td class="px-3 py-2.5 font-mono text-slate-700 whitespace-nowrap">{{ $doc->nomor_paspor ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap max-w-[140px] truncate" title="{{ $doc->jenis_keimigrasian }}">{{ $doc->jenis_keimigrasian ?? '-' }}</td>
                            ...
                            <td class="px-3 py-2.5 text-center whitespace-nowrap">
                                <form method="POST" action="{{ route('skim.destroy', $doc->id) }}" onsubmit="return confirm('Hapus data {{ addslashes($doc->nama ?? $doc->file_name) }}?')">
                                    @csrf @method('DELETE')
