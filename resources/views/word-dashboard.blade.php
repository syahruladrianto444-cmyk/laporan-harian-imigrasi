...
@section('title', 'Laporan ABK Ganda · Word')
    ...
    <section id="section-upload">
        <div x-data="uploadZone()" class="space-y-5">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 bg-blue-50/60">
                    <h2 class="text-base font-semibold text-gov-blue flex items-center gap-2">
                        ...
                        Upload Dokumen Word — Sertifikat ABK Ganda
                    </h2>
                    <p class="text-xs text-slate-500 mt-0.5">Mendukung upload multiple file sekaligus. Format: .docx · Maksimal 20MB/file · Multi-halaman otomatis terdeteksi</p>
                </div>
                <div class="p-6">
                    <form id="upload-form" action="{{ route('word.upload') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="upload-zone border-2 border-dashed border-blue-300/60 rounded-xl p-10 text-center cursor-pointer"
                            ...
                            <div class="flex flex-col items-center gap-3">
                                <div class="w-16 h-16 rounded-full bg-blue-50 flex items-center justify-center" :class="{ 'bg-blue-100': isDragging }">
                                    <svg class="w-8 h-8 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                </div>
                                <div>
                                    <p class="text-gov-blue font-semibold text-sm" x-text="isDragging ? 'Lepaskan file di sini...' : 'Klik atau Seret File Word (.docx) ke Sini'"></p>
                                    <p class="text-slate-400 text-xs mt-1">Sertifikat Bukti Pendaftaran Anak Berkewarganegaraan Ganda</p>
                                </div>
                            </div>
                            <input type="file" name="docs[]" multiple accept=".docx,.doc" x-ref="fileInput" class="hidden" @change="handleFileSelect($event)" id="word-file-input">
                        </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-48 overflow-y-auto pr-1">
                                <template x-for="(file, index) in files" :key="index">
                                    <div class="file-tag flex items-center gap-2 px-3 py-2 rounded-lg">
                                        <svg class="w-4 h-4 text-blue-500 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clip-rule="evenodd"/></svg>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-xs font-medium text-blue-600 truncate" x-text="file.name"></p>
                        ...
                        <div class="mt-5 flex items-center gap-3">
                            <button type="button" @click="submitForm()" :disabled="files.length === 0 || isUploading"
                                ...
                                <span x-text="isUploading ? 'Memproses...' : 'Proses Word'"></span>
                            ...
                            <p x-show="isUploading" x-cloak class="text-xs text-slate-500 animate-pulse">Mengekstraksi data dari Word, mohon tunggu...</p>
                        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div class="stat-card rounded-xl p-4 shadow-sm">
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total Sertifikat</p>
                ...
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #10b981;">
                    ...
                    <p class="text-2xl font-bold text-emerald-600 mt-1">{{ \App\Models\ParsedWordDocument::where('jenis_kelamin','L')->count() }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #ec4899;">
                    ...
                    <p class="text-2xl font-bold text-pink-500 mt-1">{{ \App\Models\ParsedWordDocument::where('jenis_kelamin','P')->count() }}</p>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div>
                        <h2 class="text-base font-semibold text-gov-blue flex items-center gap-2">
                            ...
                            Data Sertifikat ABK Ganda
                        </h2>
                        <p class="text-xs text-slate-500 mt-0.5">{{ $totalCount }} total record tersimpan</p>
                    </div>
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <form method="GET" action="{{ route('word.index') }}" class="flex-1 sm:flex-none">
                        ...
                        @if($totalCount > 0)
                        <a href="{{ route('word.export') }}" class="btn-primary flex items-center gap-2 px-4 py-2 text-white text-xs font-semibold rounded-lg shadow-md whitespace-nowrap">
            ...
            <div class="overflow-x-auto">
                ...
                <div class="flex flex-col items-center justify-center py-20 text-slate-400">
                    ...
                    <p class="text-xs mt-1">Upload file Word (.docx) untuk memulai ekstraksi data</p>
                </div>
                @else
                <table class="w-full text-xs">
                    <thead>
                        <tr class="bg-gov-blue text-white">
                            ...
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Kebangsaan</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Alamat</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Kota/Kabupaten</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">No. Paspor</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">File</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-slate-50/50' }}">
                            ...
                            <td class="px-3 py-2.5 font-semibold text-gov-blue whitespace-nowrap">{{ $doc->nama ?? '-' }}</td>
                            ...
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap">{{ $doc->kota_kabupaten ?? '-' }}</td>
                            <td class="px-3 py-2.5 font-mono text-slate-700 whitespace-nowrap">{{ $doc->nomor_paspor ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-400 whitespace-nowrap max-w-[120px] truncate" title="{{ $doc->file_name }}">
                                <span class="text-[10px]">{{ $doc->file_name }}</span>
                                @if($doc->page_number > 1)
                                <span class="text-[9px] bg-slate-100 px-1 py-0.5 rounded ml-1">Hal.{{ $doc->page_number }}</span>
                                @endif
                            </td>
                            <td class="px-3 py-2.5 text-center whitespace-nowrap">
                                <form method="POST" action="{{ route('word.destroy', $doc->id) }}" onsubmit="return confirm('Hapus data {{ addslashes($doc->nama ?? $doc->file_name) }}?')">
                                    @csrf @method('DELETE')
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        @else
                        <a href="{{ $documents->previousPageUrl() }}&tab=data" class="px-3 py-1.5 text-xs text-gov-blue border border-gov-blue-light/40 rounded-lg hover:bg-gov-blue-pale transition-colors">‹</a>
                        @endif
                        @foreach($documents->getUrlRange(max(1, $documents->currentPage()-2), min($documents->lastPage(), $documents->currentPage()+2)) as $page => $url)
                        <a href="{{ $url }}&tab=data" class="px-3 py-1.5 text-xs rounded-lg transition-colors {{ $page == $documents->currentPage() ? 'bg-gov-blue text-white' : 'text-gov-blue border border-gov-blue-light/40 hover:bg-gov-blue-pale' }}">{{ $page }}</a>
                        @endforeach
                        @if($documents->hasMorePages())
                        <a href="{{ $documents->nextPageUrl() }}&tab=data" class="px-3 py-1.5 text-xs text-gov-blue border border-gov-blue-light/40 rounded-lg hover:bg-gov-blue-pale transition-colors">›</a>
                        @else
                        <span class="px-3 py-1.5 text-xs text-slate-300 border border-slate-200 rounded-lg">›</span>
                        @endif
                    </div>
