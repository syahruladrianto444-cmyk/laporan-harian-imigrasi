...
@section('title', 'Laporan ABG · PDF & Word')
    ...
    <section id="section-upload">
        <div x-data="uploadZone()" class="space-y-5">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 bg-indigo-50/60">
                    <h2 class="text-base font-semibold text-gov-blue flex items-center gap-2">
                        ...
                        Upload Dokumen ABG — Sertifikat Anak Berkewarganegaraan Ganda
                    </h2>
                    <p class="text-xs text-slate-500 mt-0.5">Mendukung upload file PDF  (.docx). Maksimal 20MB/file</p>
                </div>
                <div class="p-6">
                    <form id="upload-form" action="{{ route('abg.upload') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="upload-zone border-2 border-dashed border-indigo-300/60 rounded-xl p-10 text-center cursor-pointer"
                            ...
                            <div class="flex flex-col items-center gap-3">
                                <div class="w-16 h-16 rounded-full bg-indigo-50 flex items-center justify-center" :class="{ 'bg-indigo-100': isDragging }">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-400 text-xs mt-1">Sertifikat Pendaftaran Anak Berkewarganegaraan Ganda</p>
                                </div>
                            </div>
                            <input type="file" name="docs[]" multiple accept=".pdf,.docx,.doc" x-ref="fileInput" class="hidden" @change="handleFileSelect($event)" id="abg-file-input">
                        </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-48 overflow-y-auto pr-1">
                                <template x-for="(file, index) in files" :key="index">
                                    <div class="file-tag flex items-center gap-2 px-3 py-2 rounded-lg">
                                        <svg class="w-4 h-4 text-indigo-500 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clip-rule="evenodd"/></svg>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-xs font-medium text-indigo-700 truncate" x-text="file.name"></p>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #6366f1;">
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total ABG</p>
                    <p class="text-2xl font-bold text-indigo-600 mt-1">{{ $totalCount }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #3b82f6;">
                    ...
                    <p class="text-2xl font-bold text-blue-600 mt-1">{{ \App\Models\ParsedAbgDocument::where('jenis_kelamin','L')->count() }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #ec4899;">
                    ...
                    <p class="text-2xl font-bold text-pink-500 mt-1">{{ \App\Models\ParsedAbgDocument::where('jenis_kelamin','P')->count() }}</p>
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="px-6 py-4 border-b border-slate-100 bg-indigo-50/60">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <form method="GET" action="{{ route('abg.index') }}" class="flex-1 sm:flex-none">
                        ...
                        @if($totalCount > 0)
                        <a href="{{ route('abg.export') }}" class="btn-primary flex items-center gap-2 px-4 py-2 text-white text-xs font-semibold rounded-lg shadow-md whitespace-nowrap">
                ...
                <table class="w-full text-xs">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-slate-50/50' }}">
                            ...
                            <td class="px-3 py-2.5 text-center whitespace-nowrap">
                                <form method="POST" action="{{ route('abg.destroy', $doc->id) }}" onsubmit="return confirm('Hapus data {{ addslashes($doc->nama ?? $doc->file_name) }}?')">
                                    @csrf @method('DELETE')
