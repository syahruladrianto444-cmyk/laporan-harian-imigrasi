...
@section('title', 'Laporan Izin Tinggal · PDF')
    ...
    <section id="section-upload">
        <div x-data="uploadZone()" class="space-y-5">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 bg-gov-blue-pale/40">
                    <h2 class="text-base font-semibold text-gov-blue flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                        </svg>
                        Upload File PDF Izin Tinggal
                    </h2>
                    <p class="text-xs text-slate-500 mt-0.5">Mendukung upload multiple file sekaligus. Format: PDF · Maksimal 20MB/file</p>
                </div>
                <div class="p-6">
                    <form id="upload-form" action="{{ route('pdf.upload') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="upload-zone border-2 border-dashed border-gov-blue-light/50 rounded-xl p-10 text-center cursor-pointer"
                            ...
                            <div class="flex flex-col items-center gap-3">
                                <div class="w-16 h-16 rounded-full bg-gov-blue-pale flex items-center justify-center" :class="{ 'bg-blue-100': isDragging }">
                                    <svg class="w-8 h-8 text-gov-blue-light" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-400 text-xs mt-1">ITAS · ITK · IMK · TSP/EPO · ITAP</p>
                                </div>
                            </div>
                            <input type="file" name="pdfs[]" multiple accept=".pdf" x-ref="fileInput" class="hidden" @change="handleFileSelect($event)" id="pdf-file-input">
                        </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-48 overflow-y-auto pr-1">
                                <template x-for="(file, index) in files" :key="index">
                                    <div class="file-tag flex items-center gap-2 px-3 py-2 rounded-lg">
                                        <svg class="w-4 h-4 text-gov-blue shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clip-rule="evenodd"/></svg>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-xs font-medium text-gov-blue truncate" x-text="file.name"></p>
                        ...
                        <div class="mt-5 flex items-center gap-3">
                            <button type="button" @click="submitForm()" :disabled="files.length === 0 || isUploading"
                                ...
                                class="btn-primary flex items-center gap-2 px-6 py-2.5 text-white text-sm font-semibold rounded-xl shadow-md" id="btn-process">
                                ...
                                <span x-text="isUploading ? 'Memproses...' : 'Proses PDF'"></span>
                            ...
                            <p x-show="isUploading" x-cloak class="text-xs text-slate-500 animate-pulse">Mengekstraksi data dari PDF, mohon tunggu...</p>
                        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                <div class="stat-card rounded-xl p-4 shadow-sm">
                    ...
                    <p class="text-2xl font-bold text-gov-blue mt-1">{{ $totalCount }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #8b5cf6;">
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">ITAP</p>
                    <p class="text-2xl font-bold text-purple-600 mt-1">{{ \App\Models\ParsedDocument::where('tipe_dokumen','ITAP')->count() }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #10b981;">
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">ITAS</p>
                    <p class="text-2xl font-bold text-emerald-600 mt-1">{{ \App\Models\ParsedDocument::where('tipe_dokumen','ITAS')->count() }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #f59e0b;">
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">ITK</p>
                    <p class="text-2xl font-bold text-amber-600 mt-1">{{ \App\Models\ParsedDocument::where('tipe_dokumen','ITK')->count() }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #3b82f6;">
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">IMK</p>
                    <p class="text-2xl font-bold text-blue-500 mt-1">{{ \App\Models\ParsedDocument::where('tipe_dokumen','IMK')->count() }}</p>
                </div>
                <div class="stat-card rounded-xl p-4 shadow-sm" style="border-left-color: #ef4444;">
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">TSP/EPO</p>
                    <p class="text-2xl font-bold text-red-500 mt-1">{{ \App\Models\ParsedDocument::where('tipe_dokumen','TSP-EPO')->count() }}</p>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-gov-blue-pale/40">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <form method="GET" action="{{ route('pdf.index') }}" class="flex-1 sm:flex-none" id="search-form">
                            <input type="hidden" name="tab" value="data">
                            <div class="relative">
                                <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama, paspor, kebangsaan..."
                                    ...
                                    oninput="this.form.submit()" id="search-input">
                            </div>
                        </form>
                        @if($totalCount > 0)
                        <a href="{{ route('pdf.export') }}" class="btn-primary flex items-center gap-2 px-4 py-2 text-white text-xs font-semibold rounded-lg shadow-md whitespace-nowrap" id="btn-export">
            ...
            <div class="overflow-x-auto">
                ...
                <div class="flex flex-col items-center justify-center py-20 text-slate-400">
                    ...
                    <p class="text-xs mt-1">Upload file PDF untuk memulai ekstraksi data</p>
                </div>
                @else
                <table class="w-full text-xs">
                    <thead>
                        <tr class="bg-gov-blue text-white">
                            ...
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Tgl. Permohonan</th>
                            ...
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">No. Paspor</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Exp. Paspor</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Kewarganegaraan</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Sponsor</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Alamat</th>
                            <th class="px-3 py-3 text-center font-semibold whitespace-nowrap">Tipe</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">No. Dokumen</th>
                            <th class="px-3 py-3 text-left font-semibold whitespace-nowrap">Masa Berlaku</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-slate-50/50' }}">
                            ...
                            <td class="px-3 py-2.5 text-slate-700 whitespace-nowrap">{{ $doc->tanggal_terbit ? $doc->tanggal_terbit->format('d-M-y') : '-' }}</td>
                            ...
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap">
                                @if($doc->tempat_lahir || $doc->tanggal_lahir)
                                    {{ $doc->tempat_lahir }}
                                    @if($doc->tanggal_lahir) / {{ $doc->tanggal_lahir->format('d-m-Y') }} @endif
                                @else - @endif
                            </td>
                            <td class="px-3 py-2.5 font-mono text-slate-700 whitespace-nowrap">{{ $doc->nomor_paspor ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap">{{ $doc->tanggal_expired_paspor ? $doc->tanggal_expired_paspor->format('d/m/Y') : '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap">{{ $doc->kebangsaan ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap max-w-[120px] truncate" title="{{ $doc->penjamin }}">{{ $doc->penjamin ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 max-w-[200px] truncate" title="{{ $doc->alamat }}">{{ $doc->alamat ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-center whitespace-nowrap">
                                @if($doc->tipe_dokumen)
                                <span class="inline-block px-2 py-0.5 rounded-full text-xs font-bold
                                    {{ $doc->tipe_dokumen === 'ITAS' ? 'bg-emerald-100 text-emerald-700' :
                                       ($doc->tipe_dokumen === 'ITK' ? 'bg-amber-100 text-amber-700' :
                                       ($doc->tipe_dokumen === 'ITAP' ? 'bg-purple-100 text-purple-700' :
                                       ($doc->tipe_dokumen === 'IMK' ? 'bg-blue-100 text-blue-700' :
                                       'bg-slate-100 text-slate-700'))) }}">
                                    {{ $doc->tipe_dokumen }}
                                </span>
                                @else <span class="text-slate-300">-</span> @endif
                            </td>
                            <td class="px-3 py-2.5 font-mono text-slate-700 whitespace-nowrap">{{ $doc->nomor_dokumen ?? '-' }}</td>
                            <td class="px-3 py-2.5 text-slate-600 whitespace-nowrap">{{ $doc->tanggal_expired_itas ? $doc->tanggal_expired_itas->format('d/m/Y') : '-' }}</td>
                            <td class="px-3 py-2.5 text-center whitespace-nowrap">
                            ...
                            <td class="px-3 py-2.5 text-center whitespace-nowrap">
                                <form method="POST" action="{{ route('pdf.destroy', $doc->id) }}" onsubmit="return confirm('Hapus data {{ addslashes($doc->nama ?? $doc->file_name) }}?')">
                                    @csrf @method('DELETE')
